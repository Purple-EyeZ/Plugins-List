...
const elements = {
    ...
    container: document.getElementById('plugins-container') || document.getElementById('plugins-list')
};
...
function filterPlugins(query) {
    ...
    const items = Array.from(elements.container.getElementsByClassName(
    ...
    if (isAdminPage) {
        let visibleCount = 0;
        items.forEach(item => {
            const name = normalizeText(item.querySelector('.plugin-info span:nth-child(2)')?.textContent);
            const warningMessage = item.querySelector('.warning-message')?.value;

            const matches = name?.includes(normalizedQuery) || 
                          (warningMessage && normalizeText(warningMessage).includes(normalizedQuery));
        ...
        const scoredItems = items.map(card => {
            const name = normalizeText(card.querySelector('.plugin-name').textContent);
            const description = normalizeText(card.querySelector('.plugin-description').textContent);
            const author = normalizeText(card.querySelector('.plugin-author').textContent);
        ...
        scoredItems.forEach(({ card, hasMatch }) => {
        ...
        updateSearchSubtext(query, scoredItems.filter(i => i.hasMatch).map(i => i.card));
    }
}
...
function updateSearchSubtext(query, cardsOrCount) {
    ...
    const count = typeof cardsOrCount === 'number' 
        ? cardsOrCount 
